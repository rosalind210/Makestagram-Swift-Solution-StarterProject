...
class ParseHelper {
    ...
    static func unlikePost(user: PFUser, post: Post) {
        let query = PFQuery(className: ParseLikeClass) //create query
        query.whereKey(ParseLikeFromUser, equalTo: user) //find liked post
        query.whereKey(ParseLikeToPost, equalTo: post)
        
        query.findObjectsInBackgroundWithBlock {
            (results: [AnyObject]?, error: NSError?) -> Void in
            // iterate through objects and delete those that match criteria
